"""Add last status change tracking fields to upload_jobs

Revision ID: trace_upload_jobs_last_status_v1
Revises: trace_upload_jobs_actor_v1
Create Date: 2026-01-16 00:03:00.000000
...
revision: str = "trace_upload_jobs_last_status_v1"
down_revision: Union[str, Sequence[str], None] = "trace_upload_jobs_actor_v1"
...
def upgrade() -> None:
    ...
    if "last_status_actor_api_key_id" not in cols:
        ...
        op.create_index(
            op.f("ix_upload_jobs_last_status_actor_api_key_id"),
            "upload_jobs",
            ["last_status_actor_api_key_id"],
            unique=False,
        )
        op.create_foreign_key(
            "fk_upload_jobs_last_status_actor_api_key_id_tenant_api_keys",
            "upload_jobs",
            "tenant_api_keys",
            ["last_status_actor_api_key_id"],
            ["id"],
...
def downgrade() -> None:
    ...
    if "last_status_actor_label_snapshot" in cols:
        op.drop_column("upload_jobs", "last_status_actor_label_snapshot")
    ...
    if "last_status_actor_api_key_id" in cols:
        try:
            ...
            pass
        try:
            op.drop_index(op.f("ix_upload_jobs_last_status_actor_api_key_id"), table_name="upload_jobs")
        except Exception:
            pass
        op.drop_column("upload_jobs", "last_status_actor_api_key_id")

    if "last_status_actor_kind" in cols:
        op.drop_column("upload_jobs", "last_status_actor_kind")

    if "last_status_changed_at" in cols:
        op.drop_column("upload_jobs", "last_status_changed_at")
