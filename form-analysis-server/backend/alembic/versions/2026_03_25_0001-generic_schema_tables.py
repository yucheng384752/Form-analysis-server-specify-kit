...
def _table_exists(name: str) -> bool:
    """Check if a table already exists (handles seed_stations pre-creation)."""
    conn = op.get_bind()
    result = conn.execute(
        sa.text(
...
def upgrade() -> None:
    ...
    # -- station_schemas --
    if not _table_exists("station_schemas"):
        op.create_table(
            "station_schemas",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
    ...
    # -- station_links --
    if not _table_exists("station_links"):
        op.create_table(
            "station_links",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
    ...
    # -- generic_records --
    if not _table_exists("generic_records"):
        op.create_table(
            "generic_records",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
    ...
    if not _table_exists("generic_record_items"):
        op.create_table(
            "generic_record_items",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
    ...
    # -- validation_rules --
    if not _table_exists("validation_rules"):
        op.create_table(
            "validation_rules",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
    ...
    # -- analytics_mappings --
    if not _table_exists("analytics_mappings"):
        op.create_table(
            "analytics_mappings",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
