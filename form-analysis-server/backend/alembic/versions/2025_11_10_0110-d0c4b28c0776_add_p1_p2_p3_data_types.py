...
def upgrade() -> None:
    ...
    # 創建 data_type_enum 列舉型別
    data_type_enum = sa.Enum('P1', 'P2', 'P3', name='data_type_enum')
    bind = op.get_bind()
    try:
        data_type_enum.create(bind)
    except Exception:
        # 如果已經存在，則忽略
        pass

    # ### commands auto generated by Alembic - please adjust! ###
    # 使用 try-except 忽略表格刪除錯誤（如果表格不存在）
    try:
    ...
    with op.batch_alter_table('records', schema=None) as batch_op:
        # 使用 create_type=False 避免重複創建模組
        batch_op.add_column(sa.Column('data_type', sa.Enum('P1', 'P2', 'P3', name='data_type_enum', create_type=False), nullable=True, comment='資料類型：P1(產品基本資料), P2(尺寸檢測資料), P3(追蹤編號)'))
