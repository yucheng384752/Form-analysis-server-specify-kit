...
class Record(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, comment="記錄ID"
    )

    # 基本欄位
    lot_no: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="批號，格式：7數字_2數字（如：1234567_01）"
    )
    ...
    production_date: Mapped[date | None] = mapped_column(
        Date, nullable=True, comment="生產日期，格式：YYYY-MM-DD"
    )

    # P1 專用欄位
    product_name: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="產品名稱 (P1/P3使用)"
    )

    quantity: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="數量 (P1/P3使用)"
    )

    notes: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="備註 (P1/P3使用)"
    )
    ...
    slitting_machine_number: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    machine_no: Mapped[str | None] = mapped_column(String(20), nullable=True)
    mold_no: Mapped[str | None] = mapped_column(String(20), nullable=True)
    production_lot: Mapped[int | None] = mapped_column(Integer, nullable=True)
    source_winder: Mapped[int | None] = mapped_column(Integer, nullable=True)
    product_id: Mapped[str | None] = mapped_column(String(100), nullable=True)

    p3_no: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    p3_items: Mapped[list["P3Item"]] = relationship(
        "P3Item", back_populates="record", cascade="all, delete-orphan"
    )

    # P2 明細項目關聯（一對多）
    p2_items: Mapped[list["P2Item"]] = relationship(
        "P2Item", back_populates="record", cascade="all, delete-orphan"
    )
...
# 為 lot_no 建立單獨索引
Index("ix_records_lot_no", Record.lot_no, postgresql_using="btree")

# 為 data_type 建立單獨索引
Index("ix_records_data_type", Record.data_type, postgresql_using="btree")

# 添加唯一約束防止重複資料
# 同一個 lot_no 和 data_type 組合不允許重複
from sqlalchemy import UniqueConstraint

UniqueConstraint(Record.lot_no, Record.data_type, name="uq_records_lot_no_data_type")
