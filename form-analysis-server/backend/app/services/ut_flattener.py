...
class UTFlattener:
    ...
    async def flatten_by_month(
        self, year: int, month: int, location: str = "P1+P2+P3"
    ) -> list[dict]:
        """
        按月查詢並扁平化資料
    ...
    async def _flatten_p2(self, year: int, month: int) -> list[dict]:
        ...
        query = select(P2Record).where(
            and_(P2Record.created_at >= start_date, P2Record.created_at < end_date)
        )
            ...
            if production_date_str:
                ...
                try:
                    if "-" in production_date_str:
                        ...
                        prod_date = datetime.strptime(production_date_str, "%Y%m%d")
                    timestamp = convert_yyyymmdd_to_yymmdd(
                        int(prod_date.strftime("%Y%m%d"))
                    )
                except:
                    timestamp = "000000"  # fallback
            else:
                timestamp = convert_yyyymmdd_to_yymmdd(
                    int(p2_record.created_at.strftime("%Y%m%d"))
                )
    ...
    async def _flatten_p1_p2_p3(self, year: int, month: int) -> list[dict]:
        ...
        for p3_item in p3_items:
            ...
            if p3_item.lot_no:
                ...
                p2_record_query = (
                    select(P2Record)
                    .where(P2Record.lot_no_raw == p3_item.lot_no)
                    .limit(1)
                )
            ...
            if p2_record and p2_record.lot_no_norm:
                p1_query = (
                    select(P1Record)
                    .where(P1Record.lot_no_norm == p2_record.lot_no_norm)
                    .limit(1)
                )
            ...
            for field_name, field_spec in P1_FIELD_MAPPING.items():
                if field_name in ["Machine_No.", "Semi_No."]:
                    ...
                    value = (
                        self._extract_field(p1_record, field_spec)
                        if p1_record
                        else None
                    )
            ...
            for field_name, field_spec in P2_FIELD_MAPPING.items():
                if field_name == "Semi_No.":
                    ...
                    value = (
                        self._extract_field_from_item(p2_item, field_spec)
                        if p2_item
                        else None
                    )
                    metrics[prefixed_name] = value
    ...
    def _extract_field_from_item(self, item, field_spec: str):
        """
    ...
    def _extract_field(self, record, field_spec: str, row_data: dict | None = None):
        """
