...
def get_station_config(
    ...
    numerical_cols = [
        str(x).strip() for x in feature_cfg.get("numerical", []) if str(x).strip()
    ]
    ...
    for group_name, col_list in categorical_cfg.items():
        if isinstance(col_list, list):
            categorical_cols.extend(
                [str(x).strip() for x in col_list if str(x).strip()]
            )
...
async def run_basic_statistics_from_db(
    *,
    ...
    target_col, numerical_cols, categorical_cols, id_col = get_station_config(
        config, station
    )
    ...
    if df.empty:
        logger.warning(
            "No data found for basic statistics: tenant=%s, station=%s",
            tenant_id,
            station,
        )
        return {}
...
async def run_compare_outliers_from_db(
    *,
    ...
    if df.empty:
        logger.warning(
            "No data found for compare outliers: tenant=%s, station=%s",
            tenant_id,
            station,
        )
        return {}
...
async def run_contribution_analysis_from_db(
    *,
    ...
    if df.empty:
        logger.warning(
            "No data found for contribution analysis: tenant=%s, station=%s",
            tenant_id,
            station,
        )
        return {}
    ...
    if len(present_numerical) < 2:
        logger.warning(
            "Need at least 2 numerical columns for PCA, got %d", len(present_numerical)
        )
        return {}
    ...
    if len(num_df) < 10:
        logger.warning(
            "Insufficient samples for PCA: %d (need at least 10)", len(num_df)
        )
        return {}
    ...
    analysis_ids = (
        sample_id_list[split_idx:]
        if len(sample_id_list) > split_idx
        else sample_id_list[-len(analysis_df) :]
    )

    if len(baseline_df) < 5 or len(analysis_df) < 1:
        logger.warning(
            "Insufficient data for PCA split: baseline=%d, analysis=%d",
            len(baseline_df),
            len(analysis_df),
        )
        return {}
    ...
    try:
        ...
        t2, t2_feature, t2_feature_score, t2_feature_rank = analyzer.hotelling_t2(
            std_analysis_df, pca
        )
        spe, spe_feature, spe_feature_score, spe_feature_rank = analyzer.SPE(
            std_analysis_df, pca
        )
        ...
        result = {
            "explained_variance": sel_var.tolist()
            if hasattr(sel_var, "tolist")
            else list(sel_var),
            "cumulative_variance": cum_var.tolist()
            if hasattr(cum_var, "tolist")
            else list(cum_var),
...
async def run_serialization_from_db(
    *,
    ...
    if df.empty:
        logger.warning(
            "No data found for serialization: tenant=%s, station=%s", tenant_id, station
        )
        return []
...
async def run_extraction_analysis_from_db(
    *,
    ...
    try:
        ...
        if df.empty:
            logger.warning(
                "No data for extraction analysis: tenant=%s, station=%s",
                tenant_id,
                station,
            )
            return {}
        ...
        boundary_obj = CompareOutliersResults(
            results=boundary_result if isinstance(boundary_result, dict) else {}
        )
        ...
        result = {
            "boundary_count": {
                f: merged.boundary_count.get(f, 0) for f, _ in sorted_features
            },
