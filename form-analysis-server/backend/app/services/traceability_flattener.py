...
class TraceabilityFlattener:
    ...
    async def flatten_by_month(
        ...
        return {
            ...
            "metadata": {
                ...
                "month": month,
                "compression": "auto"
                if len(flattened) >= AnalyticsConfig.AUTO_GZIP_THRESHOLD
                else "none",
                "null_handling": "explicit",
            },
        }
    ...
    async def flatten_by_product_ids(
        self, product_ids: list[str], limit: int | None = None
    ) -> dict:
        """
        ...
        query = (
            select(P3Record)
            .where(P3Record.product_id.in_(product_ids))
            .order_by(P3Record.production_date_yyyymmdd.desc())
        )
        ...
        for p3 in p3_records:
            ...
            if not rows:
                ...
                for row_data in rows:
                    ...
                    source_winder = row_data.get("source_winder") or p3.extras.get(
                        "source_winder"
                    )
    ...
    def _match_p2_by_winder(
        self, p2_records: list[P2Record], source_winder: str | None
    ) -> P2Record | None:
    ...
    def _extract_field(self, record, field_spec, row_data: dict | None = None):
        """
        從記錄中提取欄位值
        ...
        if isinstance(field_spec, str):
            ...
            if field_spec == "production_date_yyyymmdd":
                if hasattr(record, "production_date_yyyymmdd"):
                    yyyymmdd = record.production_date_yyyymmdd
