...
class UploadErrorBase(BaseModel):
    ...
    row_index: int = Field(..., ge=0, description="發生錯誤的行索引（從0開始）")
    ...
    message: str = Field(..., min_length=1, max_length=500, description="錯誤訊息描述")


class UploadErrorCreate(UploadErrorBase):
    """建立上傳錯誤的請求模型"""

    job_id: uuid.UUID = Field(..., description="關聯的上傳工作ID")
...
class UploadErrorRead(UploadErrorBase):
    ...
    id: uuid.UUID = Field(..., description="錯誤記錄ID")

    job_id: uuid.UUID = Field(..., description="關聯的上傳工作ID")

    created_at: datetime = Field(..., description="錯誤記錄建立時間")
