...
# Usage: powershell -ExecutionPolicy Bypass -File .\test-import-production-data.ps1
...
Write-Host "=== Production Data Import & Query Test ===" -ForegroundColor Cyan
Write-Host "Using Tenant ID: $TenantId" -ForegroundColor Green

# Base path for production data
...
function Import-File {
    ...
    if (-not $job.id) {
        Write-Host "  ✗ Failed to create job: $jobResponse" -ForegroundColor Red
        return $false
    }
    ...
    while ($elapsed -lt $maxWait) {
        ...
        if ($job.status -eq "FAILED") { 
            ...
            Write-Host "  Error: $($job.error_summary)" -ForegroundColor Red
            return $false
        }
    }
...
$p1Response = curl.exe -s "$BaseUrl/api/v2/records/P1?page=1&page_size=5" -H "X-Tenant-Id: $TenantId" | ConvertFrom-Json
...
$p2Response = curl.exe -s "$BaseUrl/api/v2/records/P2?page=1&page_size=5" -H "X-Tenant-Id: $TenantId" | ConvertFrom-Json
