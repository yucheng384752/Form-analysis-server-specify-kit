...
2. **跨階段追溯**：同一批次在不同階段可以通過 lot_no 關聯所有資料
...
```
**理由：**
- **資料一致性**：確保子表記錄必須對應有效的生產序號次
- **級聯刪除**：刪除批次時自動清理所有相關資料
- **參照完整性**：防止孤兒記錄產生
...
POST   /api/data/p1/{lot_no}                   # 新增P1資料
...
這個設計充分利用了 lot_no 作為核心標識符的優勢，提供了完整、高效、可擴展的生產資料管理系統。