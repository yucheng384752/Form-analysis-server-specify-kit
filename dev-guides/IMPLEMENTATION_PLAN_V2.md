...
*   **實作內容**：
    *   **Models**: `ImportJob`, `ImportFile`, `StagingRow`（存 JSONB）。
    *   **Status Enum（固定）**：
...
*   **實作內容**：
    *   驗證層採「schema-driven」為主（Pydantic 可作基本型別輔助，但避免寫死只支援 P1/P2/P3）
    *   規則：
        * required/type/range（欄位級）
        * cross-field（同列邏輯）
        * cross-row（unique）
        * cross-table（由 tenant_table_settings 開關控制：P3 是否必須對應 P2/P1）
    *   將錯誤寫回 `StagingRow.errors_json`，並提供 `/errors` API 分頁讀取
*   **測試策略**：
...
*   **測試策略**：
    *   插入一組 P1->P2->P3 資料：多條件交集能正確回傳
    *   展開 trace：缺 P1 或缺 P2/P3 也能回（空陣列 + 缺漏標記）
...
*   **測試策略**：
    *   未帶 reason → 422
    *   patch 後 row_edits 必有一筆且 before/after 正確
