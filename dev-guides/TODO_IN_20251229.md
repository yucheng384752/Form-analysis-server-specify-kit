...
- [ ] 決策並落地「legacy fallback policy」：多租戶情境一律禁用 legacy tables fallback（避免跨租戶資料外洩），並補齊所有 legacy 路由一致行為
- [ ] 完成 legacy → v2 平移計畫：前端 Query/Advanced Query 全面改打 `/api/v2/query/*`，逐步移除 `/api/query/*`
- [ ] 修復 `p3_records` / `p3_items` 關聯一致性：定義 API 讀取來源（v2 優先）與資料落點（匯入/修正）一致
- [ ] 修復 `monitor_backend.bat` / `monitor_frontend.bat` 目前硬編絕對路徑問題（需回到相對路徑，避免換機/換資料夾失效）
- [ ] 將「初始頁面不得隱性帶 admin 行為」列為不變條款：全域 wrapper 不得自動塞 `X-Admin-API-Key`，僅在 UI 明確啟用管理者模式後由呼叫點加上

## P1（資料正確性 / 路徑收斂）

- [ ] IMPORT：匯入策略收斂（只寫 v2 或雙寫 legacy+v2）並文件化；對應 migration/回填腳本的責任邊界
- [ ] SCRIPTS：整理 `form-analysis-server/scripts/local-ps1/*.ps1` 的硬編資料路徑（侑特資料）→ 改為可傳參數或讀 ENV（例如 `UT_DATA_DIR`），避免換機/換資料夾失效
