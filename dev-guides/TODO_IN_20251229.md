...
- [x] FIX: legacy 進階查詢 `winder_number` 不應回傳 P1（P0 bug）
- [x] SEC: 簡易身份驗證（目標：阻擋公有 IP 掃描/攻擊，不做完整 RBAC）
  - 最小方案 A：反向代理 / WAF / IP allowlist（部署層）
  - 最小方案 B：後端加 API key（middleware），前端帶 header（應可關閉 dev 模式）
  - 已落地（2026-01-16）：Docker Compose「部署層」預設 `AUTH_MODE=api_key`；quick-start 會自動 bootstrap 一把 key 並在 smoke test 期間帶入；程式預設仍維持 `AUTH_MODE=off`（避免測試/開發踩雷）
- [x] STABILITY: tenant bootstrap + stale tenant 自癒（避免 DB 重建後一直卡 404）

## P1（正確性 / 一致性）

- [ ] MIGRATION: legacy → v2 平移（以 v2 為準），逐步移除 legacy `/api/query/*`
- [ ] DATA: 修復 `p3_records` / `p3_items` 關聯一致性（API 讀取路徑與資料落點一致）
- [ ] IMPORT: 匯入路徑策略收斂（未來只寫 v2 或雙寫 legacy+v2；需定義與文件化）
- [x] MAINT: 修正 `datetime.utcnow()` deprecation warnings（改用 timezone-aware UTC time）

## P2（UX / 可觀測性）

- [ ] UX: 進階查詢呈現（單筆顯示 + 多筆條列）
- [ ] UX: 查詢/進階查詢按鈕視覺一致 + 展開動畫
- [ ] OBS: 匯入/查詢/修正流程事件記錄（最小可追溯 job status audit）

## P3（新功能 / 文件 / 整理）

- [ ] DOC: PDF 轉換流程與 API 契約（PDF→JSON/CSV→v2 import）
- [ ] MAINT: staging/upload/import 資料清理策略（保留天數/成功後摘要）
- [ ] I18N: 中英文介面切換

## 已完成（近期）

- [x] PDF 上傳 v1：僅保存，不轉檔/不匯入（含限制文件）
- [x] tenant not found：前端自癒 + UI toast 提示 + default tenant 改名 UT
- [x] quick-start：預設不清 DB（清 DB 需顯式 flag）
- [x] DOC: Docker 預設 auth + curl 範例同步（2026-01-16）
  - 文件更新：`form-analysis-server/README.md` 補上「Docker Compose 預設啟用 api_key auth」說明，並同步所有 `/api/*` curl 範例帶 `X-API-Key`
  - 部署更新：`form-analysis-server/docker-compose.yml` 以 `${AUTH_MODE:-api_key}` 形式在部署層預設啟用
